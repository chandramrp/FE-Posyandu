...
export default function DataBalita() {
	...
	const filtered = data.filter((b) => {
		const matchSearch =
			...
			b.namaOrtu.toLowerCase().includes(search.toLowerCase());
		const matchGender =
			filterGender === "Semua" || b.jenisKelamin === filterGender;
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (editItem) {
			...
			toast({
				title: "Berhasil",
				description: "Data balita berhasil diperbarui.",
			});
		} else {
			setData([...data, { id: Date.now(), ...form }]);
			toast({
				title: "Berhasil",
				description: "Data balita berhasil ditambahkan.",
			});
		}
		setModalOpen(false);
	};
	...
	const handleDelete = () => {
		if (deleteId) {
			...
			toast({
				title: "Berhasil",
				description: "Data balita berhasil dihapus.",
			});
		}
		setDeleteId(null);
	};
	...
	return (
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
				<div>
					...
					<p className="text-muted-foreground">
						Kelola data balita di wilayah RW
					</p>
				</div>
				<button
					onClick={openAdd}
							...
							{paginatedData.map((b) => (
								<tr
									...
									<td className="px-4 py-3">
										<span
											className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${b.jenisKelamin === "Laki-laki" ? "bg-info/10 text-info" : "bg-destructive/10 text-destructive"}`}>
											{b.jenisKelamin}
										</span>
									</td>
									<td className="px-4 py-3 text-muted-foreground">
			...
			<FormModal
				...
				title={editItem ? "Edit Data Balita" : "Tambah Data Balita"}
				...
				<div className="space-y-3">
					...
					<div>
						...
						<input
							...
							value={form.namaOrtu}
							onChange={(e) =>
								setForm({ ...form, namaOrtu: e.target.value })
							}
							className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring/30"
						/>
					</div>
			...
			<ConfirmDialog
				...
				description="Apakah Anda yakin ingin menghapus data balita ini? Tindakan ini tidak dapat dibatalkan."
			/>
		</div>
	);
}
